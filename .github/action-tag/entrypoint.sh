...
# To avoid the need for user interaction, the private key for the CI github user is added and github.com must be added to known_hosts.
echo "Authorizing SSH connection to github.com using known hosts and base64-encoded ssh private key for github machine user..."
mkdir -p /root/.ssh \
    ...
    && git config --global user.email "ci@storyblocks.com" \
    && git config --global user.name "Storyblocks CI"
